const cargaScript = () => {
  ...
  const RutaServidor = "https://pruebadeployapi.azurewebsites.net/";
  ...
  const borrarEquipo = (numID) => {
    swal({
      ...
      if (accionBorrar == "Confirmar") {
        fetch(RutaServidor + `api/equipos/${numID}`, {
          method: "DELETE",
          headers: {
  ...
  const crearEquipo = (paramEquipo, paramColor) => {
    ...
    fetch(RutaServidor + "api/equipos", {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
      },
  ...
  const editarEquipo = (paramid, paramnomEquipo, paramcolorEquipo) => {
    const objetoAEnviar = {
      id: paramid,
      nombre: paramnomEquipo,
      color: paramcolorEquipo,
    };

    fetch(RutaServidor + "api/equipos/" + paramid, {
      method: "PUT",
      headers: {
        "Content-Type": "application/json",
      },
  ...
  const eliminarJugadora = (numID, nombreEquipo) => {
    swal({
      ...
      if (accionBorrar == "Confirmar") {
        fetch(RutaServidor + "api/jugadores/" + numID, {
          method: "DELETE",
          headers: {
            "Content-Type": "application/json",
          },
  ...
  const crearJugadora = (
    ...
    console.log(JSON.stringify(objetoAEnviar));

    fetch(RutaServidor + "api/jugadores", {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
      },
      ...
      .then((e) => {
        ...
        swal("La jugadora fue agregada con Exito", {
          ...
          if (
            !document
              .getElementById("listado-jugadoras")
  ...
  const cargarJugadoras = (nombreEquipo) => {
    fetch(RutaServidor + "api/jugadores/multiple/" + nombreEquipo)
      .then((res) => res.json())
      .then((res) => {
        document.getElementById("listado-jugadoras").classList.remove("d-none");
        const contenedorJugadoras = document.getElementById(
          "listado-jugadoras-contenedor"
        );
        ...
        if (banderaVectorNulo) {
          ...
          contenedorJugadoras.getAttribute(
            "id",
            "listado-jugadoras-contenedor"
          );
        } else {
          ...
          document.querySelectorAll("#eliminar-jugadora").forEach((btn) => {
            btn.addEventListener("click", (e) => {
              ...
              eliminarJugadora(
                elementoJugadora.getAttribute("data-id"),
                elementoJugadora.getAttribute("data-equipo")
              );
            });
          });
          ...
          document.querySelectorAll("#editar-jugadora").forEach((btn) => {
            btn.addEventListener("click", (e) => {
              ...
              const nombreJugadora =
                elementoJugadora.children[1].children[0].children[0]
                  .textContent;
              const apellidoJugadora =
                elementoJugadora.children[1].children[0].children[1]
                  .textContent;
              const dniJugadora =
                elementoJugadora.children[1].children[2].children[1]
                  .textContent;
              const idJugadora = elementoJugadora.getAttribute("data-id");
              const equipoJugadora =
                elementoJugadora.getAttribute("data-equipo");

              const formulario = document.getElementById(
                "formulario-edicion-jugadora"
              );
  ...
  const editarJugadora = (
    ...
    fetch(RutaServidor + "api/jugadores/" + idJugadora, {
      method: "PUT",
      headers: {
        "Content-Type": "application/json",
      },
      ...
      if (res.status < 400) {
        ...
        const form_Jugadora_editar = document.getElementById(
          "formulario-edicion-jugadora"
        );
  ...
  document.getElementById("crear-nueva-jugadora").addEventListener(
    "click",
    (e) => {
      e.preventDefault();
      const nombreJugadora =
  ...
  const input_nombreEquipo_editar = document.getElementById(
    "input-nombreEquipo-Editar"
  );
  const input_colorEquipo_editar = document.getElementById(
    "input-colorEquipo-Editar"
  );
  form_equipo_editar.addEventListener(
    "submit",
    (e) => {
      e.preventDefault();
  ...
  boton_cancelar_editar.addEventListener(
    "click",
    (e) => {
      e.preventDefault();
      mostrarFormularioEquiposEditar("esconder");
  ...
  const input_nombreEquipo_crear = document.getElementById(
    "input-nombreEquipo-crear"
  );
  const input_colorEquipo_crear = document.getElementById(
    "input-colorEquipo-crear"
  );
  form_equipo_crear.addEventListener(
    "submit",
    (e) => {
      e.preventDefault();
  ...
  boton_cancelar_crear.addEventListener(
    "click",
    (e) => {
      e.preventDefault();
      mostrarFormularioEquiposcrear();
  ...
  const btn_cancelar_edicion = document.getElementById(
    "cancelar-editar-jugadora"
  );
  ...
  btn_submit_edicion.addEventListener("click", () => {
    const idJugadora =
      btn_submit_edicion.parentElement.parentElement.getAttribute("data-id");
  ...
  const cargarequipos = () => {
    ...
    const equipos = fetch(RutaServidor + "api/equipos")
      .then((res) => res.json())
      .then((res) => {
        ...
        menu_equipos.innerHTML = `
            ...
            `;
        const menu_equipos_edicion = document.getElementById(
          "selector-equipo-edicion"
        );
        ...
        for (var equipo of res) {
          ...
          const htmlAAgregar = `
                <div class="card-body position-relative" data-id="${equipo.id}">
                        <div class="card pt-5 pb-2">
                          ...
                          <div class="card-body">
                            <h5 class="card-title d-flex justify-content-center">${equipo.nombre}</h5>
          ...
          const opcionAAgregarCrear = `<option value="${equipo.id}">${equipo.nombre}</option>`;
          const opcionMenuEquiposCrear = document.createElement("option");
          opcionMenuEquiposCrear.innerHTML = opcionAAgregarCrear;

          const opcionAAgregarEditar = `<option value="${equipo.id}">${equipo.nombre}</option>`;
          ...
          btn.addEventListener(
            ...
            (e) => {
              ...
              if (e.target.classList.contains("far")) {
                ...
                mostrarFormularioEquiposEditar(
                  "mostrar",
                  nombre_equipo,
                  color_equipo,
                  id_equipo
          ...
          btn.addEventListener(
            ...
            (e) => {
              ...
              if (e.target.classList.contains("far")) {
                ...
                const panelJugadoras =
                  document.getElementById("listado-jugadoras");
  ...
  const flechaColapsarFormulario = document.getElementById(
    "flecha-colapsador-formularioJugadora"
  );
  ...
  const flechaColapsarListado = document.getElementById(
    "flecha-colapsador-listadoJugadoras"
  );
